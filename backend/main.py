...
def run_brca1_analysis():
    ...
    print("torch:", torch.__version__, "CUDA:", torch.version.cuda, file=sys.stderr)
    ...
    brca1_df = pd.read_excel('/evo2_repo/notebooks/brca1/41586_2018_461_MOESM3_ESM.xlsx', header=2)
    ...
    with gzip.open('/evo2_repo/notebooks/brca1/GRCh37.p13_chr17.fna.gz', 'rt') as handle:
    ...
    auroc = roc_auc_score(y_true=y_true, y_score=-brca1_subset['evo2_delta_score'])
    print(f'AUROC: {auroc}')

    # --- Calculate threshold START ---
    fpr, tpr, thresholds = roc_curve(y_true=y_true, y_score=-brca1_subset['evo2_delta_score'])
    ...
    # Mask medians from each distribution
    sns.boxplot(
        showmeans=True,
        meanline=True,
    ...
    plt.xlabel('Delta likelihood score, Evo2')
...
def analysis_variant(relative_pos_in_window, reference, alternative, window_seq, model):
