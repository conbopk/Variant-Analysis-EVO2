...
export default function KnownVariants({
    refreshVariantsAction,
    showComparisonAction,
    updateClinvarVariantAction,
    clinvarVariants,
    isLoadingClinvar,
    clinvarError,
    genomeId,
  ...
  refreshVariantsAction: () => void;
  showComparisonAction: (variant: ClinvarVariant) => void;
  updateClinvarVariantAction: (id: string, newVariant: ClinvarVariant) => void;
  ...
  const analyzeVariant = async (variant: ClinvarVariant) => {
    ...
    updateClinvarVariantAction(variant.clinvar_id, {...variant, isAnalyzing: true})
    ...
    try {
      ...
      updateClinvarVariantAction(variant.clinvar_id, updatedVariant);

      showComparisonAction(updatedVariant);
    } catch (e) {
      updateClinvarVariantAction(variant.clinvar_id, {
        ...variant,
        isAnalyzing: false,
      ...
      <Card className='gap-0 border-none bg-white py-0 shadow-sm'>
        ...
        <CardContent className='pb-4'>
          ...
          {isLoadingClinvar ? (
              ...
              <div className='h-96 max-h-96 overflow-y-scroll overflow-x-auto rounded-md border border-[#3c4f3d]/5'>
                <Table>
                  ...
                  <TableBody>
                    {clinvarVariants.map((variant) => (
                        ...
                        >
                          ...
                          <TableCell className='py-2 text-xs'>
                            <div className={`w-fit rounded-md px-2 py-1 text-center font-normal ${getClassificationColorClasses(variant.classification)}`}>
                                  ...
                                  .includes("single nucleotide") ? (
                                      !variant.evo2Result ? (
                                          ...
                                          >
                                            {variant.isAnalyzing ? (
                                                <>
                                                  <span className='mr-1 inline-block h-3 w-3 animate-spin rounded-full border-2 border-[#3c4f3d]/30 border-t-[#3c4f3d]'/>
                                                  Analyzing...
                                                </>
                                            ) : (
                                                <>
                                          ...
                                          <Button
                                            ...
                                            onClick={() => showComparisonAction(variant)}
                                          >
