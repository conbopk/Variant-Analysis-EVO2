        ...
        <div className='max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-lg bg-white'>
            ...
            {comparisonVariant?.evo2Result && (
                <div className='space-y-6'>
                    ...
                    <div className='grid gap-4 md:grid-cols-2'>
                      ...
                      <div>
                        <div className='space-y-2'>
                          <div className='flex'>
                            <span className='w-28 text-xs text-[#3c4f3d]/70'>
                              Variant (from NCBI database of Clinical Variant which table show):
                            </span>
                            <span className='font-mono text-xs'>
                              {(() => {
                                const regex = /(\w)>(\w)/;
                                const match = regex.exec(comparisonVariant.title);
                                if (match?.length === 3) {
                          ...
                          <div className='flex'>
                            <span className='w-28 text-xs text-[#3c4f3d]/70'>
                              Variant (from UCSC database - EVO2 model used to get reference genome):
                            </span>
                            <span className='font-mono text-xs'>
                              {(() => {
                                const regex = /(\w)>(\w)/;
                                ...
                                if (match?.length === 3) {
                                  ...
                                  return (
                                      <>
                                        <span className={getNucleotideColorClass(comparisonVariant.evo2Result?.reference)}>
                          ...
                          <div className='flex items-center'>
                            ...
                            >
                              ...
                              <ExternalLink className='ml-1 inline-block w-3 h-3 text-[#de8246]' />
                            </a>
                          </div>
                        </div>
                      </div>
                  ...
                  <div>
                    ...
                    <p className='mb-4 text-xs text-[#3c4f3d] leading-relaxed'>
                      Note: Analysis Comparison <span className='font-medium'>only makes sense when the reference gene from the
                      NCBI database
                      of the Clinical Variant (as shown in the table) matches the reference gene from the {" "}
                      UCSC database
                      that the EVO2 model uses to retrieve the reference gene</span>. Although they have the same genome and the same position,
                      these two databases sometimes return different reference genes, so the results from the model or from the NCBI database cannot be compared.
                    </p>
                    <div className='rounded-md border border-[#3c4f3d]/10 bg-white p-4'>
                      <div className='grid gap-4 md:grid-cols-2'>
                        ...
                        <div className='rounded-md bg-[#e9eeea]/50 p-4'>
                          <h5 className='mb-2 flex items-center gap-2 text-xs font-medium text-[#3c4f3d]'>
                            <span className='flex h-5 w-5 items-center justify-center rounded-full bg-[#3c4f3d]/10'>
                              <span className='h-3 w-3 rounded-full bg-[#3c4f3d]' />
                            </span>
                            ClinVar Assessment
                          </h5>
                          <div className='mt-2'>
                        ...
                        <div className='rounded-md bg-[#e9eeea]/50 p-4'>
                          <h5 className='mb-2 flex items-center gap-2 text-xs font-medium text-[#3c4f3d]'>
                            <span className='flex h-5 w-5 items-center justify-center rounded-full bg-[#3c4f3d]/10'>
                              <span className='h-3 w-3 rounded-full bg-[#de8246]' />
                            </span>
                            Evo2 Prediction
                          </h5>
                          <div className='mt-2'>
                          ...
                          <div className='mt-3'>
                            ...
                            <div className='text-sm font-medium'>
                              {comparisonVariant.evo2Result.delta_score.toFixed(8)}
                            </div>
                            <div className='text-xs text-[#3c4f3d]/60'>
                              {comparisonVariant.evo2Result.delta_score < 0
                                ? "Negative score indicated loss of function"
                      ...
                      <div className='mt-4 rounded-md bg-[#e9eeea]/20 p-3 text-xs leading-relaxed'>
                        <div className='flex items-center gap-2'>
                          <span className={`flex h-5 w-5 items-center justify-center rounded-full ${comparisonVariant.classification.toLowerCase() === comparisonVariant.evo2Result.prediction.toLowerCase() ? "bg-green-100" : "bg-yellow-100"}`}>
          ...
          <div className='flex justify-end border-t border-[#3c4f3d]/10 bg-[#e9eeea]/30 p-4'>
            <Button
              variant={'outline'}
              onClick={onClose}
              className='cursor-pointer border-[#3c4f3d]/10 bg-white text-[#3c3f3d] hover:bg-[#e9eeea]/70'
            >
              Close
            </Button>
          </div>
