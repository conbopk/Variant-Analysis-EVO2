...
export function GeneSequence({
  ...
  startPosition,
  endPosition,
  onStartPositionChangeAction,
  onEndPositionChangeAction,
  sequenceData,
  sequenceRange,
  isLoading,
  error,
  onSequenceLoadRequestAction,
  onSequenceClickAction,
  maxViewRange,
}: {
  ...
  endPosition: string;
  onStartPositionChangeAction: (value: string) => void;
  onEndPositionChangeAction: (value: string) => void;
  sequenceData: string;
  sequenceRange: {start: number; end: number} | null;
  isLoading: boolean
  error: string | null;
  onSequenceLoadRequestAction: () => void;
  onSequenceClickAction: (position: number, nucleotide: string) => void;
  maxViewRange: number;
}) {
      ...
      if (isDraggingStart) {
        if (!isNaN(currentEndNum)) {
          if (currentEndNum - newPosition + 1 > maxViewRange) {
            onStartPositionChangeAction(String(currentEndNum - maxViewRange + 1));
          } else if (newPosition < currentEndNum) {
            onStartPositionChangeAction(String(newPosition));
          }
        }
      } else if (isDraggingEnd) {
        if (!isNaN(currentStartNum)) {
          if (newPosition - currentStartNum + 1 > maxViewRange) {
            onEndPositionChangeAction(String(currentStartNum + maxViewRange - 1));
          } else if (newPosition > currentStartNum) {
            onEndPositionChangeAction(String(newPosition));
          }
        }
      } else if (isDraggingRange) {
        if (!dragStartX.current) return;
        const pixelPerBase = sliderWidth / geneSize;
        const dragDeltaPixels = relativeX - dragStartX.current.x;
        const dragDeltaBases = Math.round(dragDeltaPixels / pixelPerBase);
        ...
        onStartPositionChangeAction(String(newStart));
        onEndPositionChangeAction(String(newEnd));
      }
    };
  ...
  const formattedSequence = useMemo(() => {
    ...
    for (let i = 0; i < sequenceData.length; i += BASES_PER_LINE) {
      ...
      for (let j = 0; j < chunk.length; j++) {
        ...
        colorizedChars.push(
            <span
              key={j}
              onClick={() => onSequenceClickAction(nucleotidePosition, nucleotide)}
              onMouseEnter={(e) => {
    ...
    return lines;
  }, [sequenceData, sequenceRange, onSequenceClickAction]);
                ...
                <div className='space-y-4'>
                  <div className='relative'>
                    ...
                    >
                      ...
                      {/*Start handle*/}
                      <div
                          className='absolute top-1/2 -translate-y-1/2 flex h-6 w-6 -translate-x-1/2 cursor-grab items-center justify-center rounded-full border-2 border-[#3d4f3d] bg-white shadow active:cursor-grabbing'
                          style={{left: `${sliderValues.start}%`}}
                          onMouseDown={(e) => handleMouseDown(e, "start")}
                      >
                        <div className='h-3 w-1 rounded-full bg-[#3c4f3d]' />
                      </div>
                  ...
                  <div className='flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0'>
                    <div className='flex items-center gap-2'>
                      <span className='text-xs text-[#3c3f3d]/70'>Start:</span>
                      <Input
                          value={startPosition}
                          onChange={(e) => onStartPositionChangeAction(e.target.value)}
                          type="text"
                          inputMode='numeric'
                          pattern='[0-9]*'
                    ...
                    <Button
                      size='sm'
                      disabled={isLoading}
                      onClick={onSequenceLoadRequestAction}
                      className='h-7 w-full sm:w-auto cursor-pointer bg-[#3c4f3d] text-xs text-white hover:bg-[#3c4f3d]/90'
                    >
                      {isLoading ? "Loading..." : "Load sequence"}
                    </Button>
                    <div className='flex items-center gap-2'>
                      <span className='text-xs text-[#3c3f3d]/70'>End:</span>
                      <Input
                          value={endPosition}
                          onChange={(e) => onEndPositionChangeAction(e.target.value)}
                          type="text"
                          inputMode='numeric'
                          pattern='[0-9]*'
                          className='h-7 w-full border-[#3c4f3d]/10 text-xs sm:w-28'
          ...
          <div className='w-full rounded-md bg-[#e9eeea]/50 p-3'>
            {isLoading ? (
              <div className='flex items-center justify-center py-8'>
                <div className='h-5 w-5 animate-spin rounded-full border-2 border-gray-300 border-t-[#3c4f3d]' />
              </div>
            ) : sequenceData ? (
